...
public class ThreadPoolUtils {
    ...
    /**
     * 提交一个Callable任务用于执行
     * <p>如果想立即阻塞任务的等待，则可以使用{@code result = exec.submit(aCallable).getCacheDao();}形式的构造。</p>
     *
     * @param task 任务
     * @param <T>  泛型
     * @return 表示任务等待完成的Future, 该Future的{@code getCacheDao}方法在成功完成时将会返回该任务的结果。
     */
     ...
     *
     * @param task 任务
     * @return 表示任务等待完成的Future, 该Future的{@code getCacheDao}方法在成功完成时将会返回null结果。
     */
     ...
     * @param delay   延迟时间
     * @param unit    单位
     * @return 表示挂起任务完成的ScheduledFuture，并且其{@code getCacheDao()}方法在完成后将返回{@code null}
     */
     ...
     * @param unit         时间单位
     * @return 表示挂起任务完成的ScheduledFuture，并且其{@code getCacheDao()}方法在取消后将抛出异常
     */
